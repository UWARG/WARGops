<template>
  <v-card>
    ...
    <v-form v-model="valid" :lazy-validation="false" ref="form" class="p-4">
      <div class="flex">
        <v-text-field class="mr-2 flex-1" label="Waterloo ID" variant="outlined" v-model="accountInfo.waterloo_id"
          :rules="[rules.required]" />
        <v-text-field class="ml-2 flex-1" label="Point of Contact" variant="outlined"
          v-model="accountInfo.point_of_contact" :rules="[rules.required, rules.email]" />
      </div>
      <div class="flex mb-4">
        <div class="flex-1 mr-2">
          <v-text-field label="Name" variant="outlined" :rules="[rules.required]" v-model="accountInfo.name" />
          <v-autocomplete label="Term" variant="outlined" :rules="[rules.required]" v-model="accountInfo.term"
            :items="dates" placeholder="Select..." />
        ...
        <div class="flex-1 ml-2">
          ...
          <v-date-picker class="inline-block flex-1 w-full" v-model="accountInfo.expiry_date" color="yellow" is-dark />
        </div>
      </div>
    </v-form>
  </v-card>
